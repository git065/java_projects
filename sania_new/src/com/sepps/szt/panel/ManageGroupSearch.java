...
import com.sepps.szt.domain.ManageGroup;
...
public class ManageGroupSearch extends JPanel {
	private ManageGroup person = null;
	// private ManagerGroupColumn manager_column = null;
	private MvbOracleConnection mvb = MvbOracleConnection.getInstance();
	private ManagerGroupColumn manager_column = ManagerGroupColumn
			.getInstance();
	...
	void initPerson() {
		person = new ManageGroup();
		manager_column = new ManagerGroupColumn();
		// manager_column.open();
	}
	...
	public void createTable(String tlf) throws Exception {
		...
		DatabaseTableColumn[] columns = manager_column.getColumns();

		String keyField = manager_column.getKeyField();
	...
	private JPanel top1Panel() {
		...
		labelTlf = new JLabel("Группа управления :");
		с.gridx = 0;
		с.gridy = 0;
		с.gridwidth = 1;
		с.gridheight = 1;
...
class ManageTableModel extends DatabaseTableModel {
	public ManageTableModel(ArrayList data, DatabaseTableColumn[] columns,
